...
public class BaseApiController : ControllerBase
{
    ...
    protected ActionResult GetResult<T>(ResultValidator<T> result)
    {
        ...
        if (!result.IsSuccess && result.Value == null && result.StatusCode == 404) return NotFound(CreateResponse(result.StatusCode, result.Message));

        return BadRequest(CreateResponse(result.StatusCode, result.Message));
    }
